...
- **테스트 모드 원본 데이터 보존**: 판정에 사용한 feature vector(정적)/손끝 궤적(모션)을 `raw_vector`/`raw_window`로, **feature로 가공되기 전 MediaPipe 21관절 원본 좌표**까지 `raw_landmarks`/`raw_landmarks_window`로 함께 저장 — 분류 알고리즘·모델은 물론 feature 설계 자체가 바뀌어도 과거 세션 데이터를 처음부터 재계산/재채점할 수 있음. 보정 세션(`calibration.py`/`motion_calibration.py`)도 동일하게 `calibration_landmarks.npy`/`motion_calibration_landmarks.npy`(및 프로필별 사본)로 원본 좌표를 보존. `B`로 재시도해 덮어써진 이전 시도는 폐기하지 않고 `discarded_attempts`에 별도 보존
- **캡처 화면 원본 저장**: 보정 세션은 심볼당 1초 분량(약 30프레임)을 mp4 비디오로 저장(`calibration_videos/`, `motion_calibration_videos/`), 테스트 모드는 정적 심볼 스냅샷(.jpg)과 모션 심볼 클립(.mp4)을 세션별 폴더(`test_media/<참가자>_<시각>/`)에 저장 — 손 인식 모델이 바뀌어도 원본 화면부터 다시 처리할 수 있음
...
│   ├── motion_calibration_landmarks.npy / .json
│   ├── motion_calibration_videos/<symbol>_rep<n>.mp4    # 반복별 캡처 화면 원본
│   ├── cal_<name>.npy(.json) / cal_<name>_landmarks.npy(.json) / cal_<name>_videos/
│   ├── motion_cal_<name>.npy(.json) / motion_cal_<name>_landmarks.npy(.json) / motion_cal_<name>_videos/
