...
│   ├── cal_<name>.npy(.json) / cal_<name>_landmarks.npy(.json)
│   ├── motion_cal_<name>.npy(.json) / motion_cal_<name>_landmarks.npy(.json)
│   ├── calibration_profiles.json
│   └── test_results.json      # 세션마다 raw_vector/raw_window(가공된 feature) + raw_landmarks/raw_landmarks_window(원본 21관절) + discarded_attempts 포함
